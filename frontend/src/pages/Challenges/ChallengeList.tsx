...
const LANGUAGES = ["Tous", "javascript", "python", "java", "cpp"];
const DIFFICULTIES = ["Tous", "easy", "medium", "hard", "expert"];
...
const ChallengeList = () => {
  ...
  const fetchChallenges = async () => {
    ...
    try {
      ...
      if (difficulty !== "Tous") params.difficulty = difficulty;
      if (language !== "Tous")   params.language = language;
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        ...
        <span style={s.count}>{total} challenges disponibles</span>
      </div>
      ...
      <form onSubmit={handleSearch} style={s.filters}>
        ...
        <select style={s.select} value={difficulty} onChange={e => { setDifficulty(e.target.value); setPage(1); }}>
          {DIFFICULTIES.map(d => <option key={d} value={d}>{d === "Tous" ? "Toutes difficultés" : DIFFICULTY_COLORS[d]?.label}</option>)}
        </select>
        <select style={s.select} value={language} onChange={e => { setLanguage(e.target.value); setPage(1); }}>
          {LANGUAGES.map(l => <option key={l} value={l}>{l === "Tous" ? "Tous les langages" : l}</option>)}
        </select>
        ...
        <table style={s.table}>
          <thead>
            <tr>
              ...
              <th style={s.th}>Difficulté</th>
              <th style={s.th}>Langages</th>
              <th style={s.th}>Taux d'acceptation</th>
              <th style={s.th}>XP</th>
            </tr>
          </thead>
          <tbody>
            {challenges.length === 0 ? (
              <tr><td colSpan={6} style={{ ...s.td, textAlign: "center", color: "#888" }}>Aucun challenge trouvé</td></tr>
            ) : challenges.map((c, i) => {
      ...
      {totalPages > 1 && (
        <div style={s.pagination}>
          <button style={s.pgBtn} disabled={page === 1} onClick={() => setPage(p => p - 1)}>← Précédent</button>
          <span style={{ padding: "6px 14px", fontSize: 14, color: "#666" }}>Page {page} / {totalPages}</span>
          <button style={s.pgBtn} disabled={page === totalPages} onClick={() => setPage(p => p + 1)}>Suivant →</button>
        </div>
      )}
    </div>
  );
