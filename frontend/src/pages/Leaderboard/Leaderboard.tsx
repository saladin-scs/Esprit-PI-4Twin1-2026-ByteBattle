      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-6">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center">
            ...
            <button className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-semibold py-3 px-6 rounded-lg transition-all duration-300 transform hover:scale-105 shadow-lg">
              View Profile
            </button>
          </div>
        </div>
              ...
              {users.slice(0, 3).map((user, index) => (
                <div key={user.id} className={`rounded-xl p-6 ${index === 0 ? 'md:order-2' : index === 1 ? 'md:order-1' : 'md:order-3'}`}>
                  <div className={`${getRankColor(user.rank)} p-8 rounded-xl text-center relative`}>
                    {user.rank === 1 && (
                      <Crown className="w-8 h-8 text-yellow-300 absolute -top-4 left-1/2 transform -translate-x-1/2" />
                    )}
                    <div className="text-6xl font-bold mb-4">{user.rank}</div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gray-900">
                  ...
                  <th className="py-4 px-6 text-left font-semibold">Coder</th>
                  <th className="py-4 px-6 text-left font-semibold">Score</th>
                  <th className="py-4 px-6 text-left font-semibold">Problems Solved</th>
                  <th className="py-4 px-6 text-left font-semibold">Success Rate</th>
                  <th className="py-4 px-6 text-left font-semibold">Streak</th>
                  <th className="py-4 px-6 text-left font-semibold">Rank Change</th>
                </tr>
              </thead>
              <tbody>
                {filteredUsers.map(user => (
                  <tr key={user.id} className="border-b border-gray-700 hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center font-bold mr-4">
