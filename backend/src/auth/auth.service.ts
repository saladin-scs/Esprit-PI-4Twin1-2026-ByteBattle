...
async register(registerDto: RegisterDto, meta?: { ip?: string; userAgent?: string }) {
    ...
    // ✅ AJOUT : sauvegarder faceDescriptor si l'utilisateur a capturé son visage
    if (registerDto.faceDescriptor && registerDto.faceDescriptor.length > 0) {
      await this.usersService.registerFace(String(user._id), registerDto.faceDescriptor);
    }
